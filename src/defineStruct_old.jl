...
# TODO: It would be nice to define the element type of the Struct, e.g., TStruct{Float64}
type TStruct{T} <: Entry{T}
    d::Char
    dict::Dict{Symbol,Entry{T}}
    TStruct{R}(p::Char='/',pict::Dict{Symbol,Entry{R}}=Dict{Symbol,Entry{R}}()) where R = new(p,pict)
    function TStruct{R}(p::AbstractString,pict::Dict{Symbol,Entry{R}}=Dict{Symbol,Entry{R}}()) where R
        @assert length(p)==1 "A TStruct delimiter must be one character; got $(length(p)): "*join(p,",")
        new(p[1],pict)
    end
end
TStruct(o...)=TStruct{Any}(o...)
type TValue{T} <: Entry{T}; value::T; end

type Struct <: Entry{Any}
    d::Char
    dict::Dict{Symbol,Entry}
    Struct(p::Char='/',pict::Dict{Symbol,Entry}=Dict{Symbol,Entry}())=new(p,pict)
    function Struct(p::AbstractString,pict::Dict{Symbol,Entry}=Dict{Symbol,Entry}())
        @assert length(p)==1 "A Struct delimiter must be one character; got $(length(p)): "*join(p,",")
        new(p[1],pict)
    end
end
# const Value=TValue{Any}
type Value <: Entry{Any}; value::Any; end
const AbstractValue=Union{Value,TValue}
...
function insertkeyval!{T,N,S<:Symbol}(s::TStruct{T},keys::NTuple{N,S},val)
    if N==1
        s.dict[keys[1]]=TValue{T}(val)
    else
        haskey(s.dict,keys[1])||(s.dict[keys[1]]=TStruct{T}(s.d))
        insertkeyval!(s.dict[keys[1]],keys[2:N],val)
    end
end
function insertkeyval!{N,S<:Symbol}(s::Struct,keys::NTuple{N,S},val)
    if N==1
        s.dict[keys[1]]=Value(val)
    else
        haskey(s.dict,keys[1])||(s.dict[keys[1]]=Struct(s.d))
        insertkeyval!(s.dict[keys[1]],keys[2:N],val)
    end
end
...
If `key` is an `AbstractString` it is first converted to a `Vector{Symbol}` via `splitkey(key)`.
"""
getkey(s::AbstractValue,o...)=throw(error("getkey does not work on $(typeof(s)) objects"))
...
function getkey{N,S<:Symbol}(s::AbstractStruct,k::NTuple{N,S},def=nothing)
    ...
    isa(kval,AbstractValue) && (kval=kval.value) # strip off the type wrapper if it is a Value object
    return kval
end
function getkey(s::AbstractStruct,k::Symbol,def=nothing)
  kval=get(s.dict,k,def)
  isa(kval,AbstractValue) && (kval=kval.value)
  return kval
end
...
If `key` is an `AbstractString` it is first converted to a `Vector{Symbol}` via `splitkey(key)`.
"""
structhaskey(s::AbstractValue,o...)=false # The Value type has no "keys"
